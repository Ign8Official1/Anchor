...
export class VerletPhysics {
    ...
    async update(interval, elapsed) {
        ...
        const embed = typeof conf !== "undefined" && conf.embed;
        const stepsPerSecond = embed ? 90 : 360;
        const maxStepsPerFrame = embed ? 2 : 12;
        const timePerStep = 1 / stepsPerSecond;
        interval = Math.max(Math.min(interval, embed ? 1 / 30 : 1 / 60), 0.0001);
        this.timeSinceLastStep += interval;

        let steps = 0;
        while (this.timeSinceLastStep >= timePerStep && steps < maxStepsPerFrame) {
